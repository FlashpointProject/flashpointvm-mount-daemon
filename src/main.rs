...
async fn mount_device<T: BuildHasher>(device_name: &str, shared_state: Arc<LockedMountStatus<T>>) -> HTTPResponse {
...
async fn cleanup_mount<T: BuildHasher>(shared_state: &Arc<LockedMountStatus<T>>, zip_mountpt: &str, fuzzy_mountpt: &str, union_mountpt: &str) -> Option<HTTPResponse> {
...
fn remove_changing<T: BuildHasher>(key: &str, shared_state: &Arc<LockedMountStatus<T>>) -> Option<HTTPResponse> {
    match shared_state.status.lock() {
        Ok(mut mount_status) => {
...
async fn handle_subprocess<T: BuildHasher>(spawnedproc: std::io::Result<Child>, failure_key: &str, shared_state: &Arc<LockedMountStatus<T>>) -> Option<HTTPResponse> {
    match spawnedproc {
